...
// index.ts
var exports_FATE = {};
__export(exports_FATE, {
  generateTDHPage: () => generateTDHPage,
  generateTDHLayout: () => generateTDHLayout,
  default: () => TDH,
  TDHPages: () => TDHPages,
  TDHLayout: () => TDHLayout
});
module.exports = __toCommonJS(exports_FATE);
...
function listRecursiveSync(base) {
  ...
  return scan(base).map((p) => import_path.default.relative(base, p).replace(/\\/g, "/"));
}
...
async function TDHRouteGenerator() {
  const outDir = import_path.default.join(process.cwd(), ".TDH");
  const buildDir = import_path.default.join(outDir, "build");
  import_fs.default.rmSync(outDir, { recursive: true, force: true });
  import_fs.default.mkdirSync(outDir);
  import_fs.default.mkdirSync(buildDir);
  ...
  for (const rel of files) {
    const abs = import_path.default.join(base, rel);
    ...
    if (isLayout) {
      dirLayout[import_path.default.dirname(rel) === "." ? "" : import_path.default.dirname(rel)] = abs;
    } else if (is404) {
      notFound = abs;
    } else if (is500) {
  ...
  if (Object.keys(pageEntryPoints).length === 0) {
    import_fs.default.writeFileSync(import_path.default.join(outDir, "config.json"), JSON.stringify({}, null, 2));
    return;
  }
  if (!notFound) {
  ...
  try {
    ...
    for (const { name, path: specialPath } of specialPages) {
      ...
      const module2 = (await import(outputFile)).default;
      if (!module2) {
    ...
    for (const [rel, abs] of Object.entries(pageEntryPoints)) {
      ...
      const segments = import_path.default.dirname(rel).split(import_path.default.sep);
      const layouts = [];
  ...
  import_fs.default.writeFileSync(import_path.default.join(outDir, "config.json"), JSON.stringify(finalConfig, null, 2));
}
function findOutputFile(absInputPath, result) {
  const relativeInput = import_path.default.relative(process.cwd(), absInputPath).replace(/\\/g, "/");
...
async function getConfig() {
  ...
  const configDir = import_path2.default.join(process.cwd(), ".TDH", "config.json");
  const config = JSON.parse(await import_promises.default.readFile(configDir, {
    encoding: "utf-8"
  }));
...
class TDH {
  static __init__(config) {
    ...
    RootConfig.TDH_RENDER_PATH = config?.TDH_RENDER_PATH || import_path3.default.join(process.cwd(), "views");
